...
<%
	...
	if(word == null) {
		searchSql = "SELECT board_no boardNo, board_title boardTitle FROM board ORDER BY board_no ASC LIMIT 0, 10";
		searchStmt = conn.prepareStatement(searchSql);
	} else {
		searchSql = "SELECT board_no boardNo, board_title boardTitle FROM board WHERE board_content LIKE ? ORDER BY board_no ASC LIMIT 0, 10";
		searchStmt = conn.prepareStatement(searchSql);
		searchStmt.setString(1, "%"+word+"%");
	}

	/* 검색단어를 포함하는 전체 행의 수 -> 세서 바로 옆에 출력*/
	/* 내용에 검색단어를 포함하는 목록 -> 값 보내서 리스트에 출력*/
